...
```
   ...
   Versions: `/@handle/slug?v=N` plays an older ready version (`loadWatch(handle, slug, versionNo)` → `GameFull.playing`, `liveVersionId`; the page keys `WatchView` by version). Only the live version ranks: `/api/runs/score` answers `flagged: "old_version"` for others. The edit page's `VersionManager` (`components/habiv/version-manager.tsx`, data in `GameEditData.versions`/`currentVersionId`/`storage`) edits notes (and model/tool/prompt of non-live versions via `setVersionMeta`), makes a version live (`publishVersion`) and deletes non-live ones (`deleteVersion` → `lib/versions/cleanup.ts`, shared with MCP `delete_version`). Nightly pruning is off by default (`KEEP_VERSIONS_PER_GAME`), so old versions stay until the creator deletes them.
