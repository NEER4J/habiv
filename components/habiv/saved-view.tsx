...
import { BentoGrid, BentoStack, EmptyCell, GameCard, GameCards, PageHead } from "./game-card";
import { useShell } from "./shell-context";

export function SavedView({ games, daily }: { games: Game[]; daily: Game | null }) {
  const { savedIds, sessionReady } = useShell();
  // The server list is the viewer's saves; once the session is live, unsaving drops a card immediately.
  ...
  return (
    <BentoStack>
      ...
      {list.length ? (
        <GameCards>
          {list.map((g) => (
            <GameCard key={g.id} game={g} showModel={false} showStats={false} />
          ))}
        </GameCards>
      ) : null}
    </BentoStack>
