...
export type ViewerState = { liked: Set<string>; disliked: Set<string>; saved: Set<string>; following: Set<string> };
...
export async function getViewerState(supabase: SupabaseClient<Database>, gameIds: string[], creatorIds: string[] = []): Promise<ViewerState> {
  const empty: ViewerState = { liked: new Set(), disliked: new Set(), saved: new Set(), following: new Set() };
  const { data: claims } = await supabase.auth.getClaims();
  const uid = claims?.claims?.sub;
  if (!uid || (!gameIds.length && !creatorIds.length)) return empty;
  const [likes, dislikes, saves, follows] = await Promise.all([
    ...
    gameIds.length ? supabase.from("dislikes").select("game_id").eq("user_id", uid).in("game_id", gameIds) : Promise.resolve({ data: [] as { game_id: string }[] }),
  ...
  return {
    ...
    disliked: new Set((dislikes.data ?? []).map((r) => r.game_id)),
