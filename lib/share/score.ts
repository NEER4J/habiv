...
export function signScoreShare(s: ScoreShare): string {
  const body = Buffer.from(JSON.stringify([s.gameId, s.score, s.rank, s.total, s.rounds, s.playedMs, s.name, s.at])).toString("base64url");
...
export function readScoreShare(token: string | null | undefined): ScoreShare | null {
  ...
  try {
    ...
    const [gameId, score, rank, total, rounds, playedMs, name, at] = JSON.parse(Buffer.from(body, "base64url").toString("utf8")) as unknown[];
