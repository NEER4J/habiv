...
import { isProtectedPath, welcomePath } from "../auth/protected";
...
export async function updateSession(request: NextRequest) {
  ...
  if (needsAuth && user && request.cookies.get(HANDLE_SET_COOKIE)?.value !== user.sub) {
    ...
    if (profile && !profile.handle_set) {
      // Home with the profile-setup popup open; it carries on to this page once a handle is picked.
      const redirect = NextResponse.redirect(new URL(welcomePath(pathname + request.nextUrl.search), request.url));
