...
const moods = {
  wild: { color: [155, 210, 119], title: 'the beautifully wild.', word: 'fern-green', twist: 3 },
  calm: { color: [140, 204, 171], title: 'a quieter kind of magic.', word: 'teal', twist: 2 },
  dreamy: { color: [191, 215, 166], title: 'a little lost in the leaves.', word: 'sage-green', twist: 4 },
  electric: { color: [213, 231, 126], title: 'a spark of the sun.', word: 'lime', twist: 5 }
};
...
function draw(context, w, h, state, t) {
  ...
  context.fillStyle = '#0b2018'; context.fillRect(0, 0, w, h);
  const glow = context.createRadialGradient(w*.5,h*.46,0,w*.5,h*.46,w*.51);
  glow.addColorStop(0,'#213e2b'); glow.addColorStop(1,'#0b2018'); context.fillStyle=glow;context.fillRect(0,0,w,h);
  ...
  const scale=w*.285,cx=w*.5,cy=h*.465,spin=t*.035+g.offset,tilt=g.tilt;
...
$('#surprise').addEventListener('click',()=>{const words=['Fern','Morning dew','Daydreamer','Wildflower','Little river','Afterglow','Wild heart','Evergreen'];let i=crypto.getRandomValues(new Uint32Array(2));update({name:words[i[0]%words.length],mood:Object.keys(moods)[i[1]%4]});});
function pauseLabel(){$('.forest').classList[paused?'add':'remove']('still');$('#pause').textContent=paused?'▶':'Ⅱ';$('#pause').setAttribute('aria-label',paused?'Play animation':'Pause animation');$('#pause').setAttribute('aria-pressed',String(paused));}
$('#pause').addEventListener('click',()=>{paused=!paused;pauseLabel();});pauseLabel();
$('#save').addEventListener('click',()=>{const out=document.createElement('canvas');out.width=1600;out.height=1800;const c=out.getContext('2d');draw(c,1600,1700,current,0);c.fillStyle='#0b2018';c.fillRect(0,1510,1600,290);c.fillStyle='#a4ada0';c.font='18px monospace';c.fillText('HABIV / FIND YOUR NATURAL RHYTHM',80,91);c.fillText(current.mood.toUpperCase()+' FREQUENCY',80,1580);c.fillStyle='#eeeee4';c.font='italic 64px Georgia';c.fillText(current.name?current.name+'’s orbit.':moods[current.mood].title,80,1670,1430);c.fillStyle='#a4ada0';c.font='20px monospace';c.fillText('habiv — a little closer to nature',80,1740);out.toBlob(blob=>{if(!blob){announce('Couldn’t create the image. Please try again.');return;}const url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download='habiv-'+(current.name.replace(/[^a-z0-9]/gi,'-')||'orbit')+'.png';a.click();setTimeout(()=>URL.revokeObjectURL(url),5000);announce('Your orbit is ready to keep.');},'image/png');});
$('#share').addEventListener('click',async()=>{const url=new URL(location.href);url.search='';url.hash=new URLSearchParams({name:current.name,mood:current.mood}).toString();const data={title:'Habiv — A little closer to nature',text:current.name?`Meet ${current.name}’s orbit. What does yours look like?`:'A quiet moment in green. Find your rhythm.',url:url.href};try{if(navigator.share){await navigator.share(data);}else if(navigator.clipboard){await navigator.clipboard.writeText(url.href);announce('Orbit link copied. Send a little quiet.');}else{window.prompt('Copy your orbit link:',url.href);}}catch(e){if(e.name!=='AbortError'){window.prompt('Copy your orbit link:',url.href);}}});
